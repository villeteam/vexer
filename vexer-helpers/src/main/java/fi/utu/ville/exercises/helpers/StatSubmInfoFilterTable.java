 ...
 * </p>
 * <p>
 * This class can be used to provide filtering functionalities and to possible
 * also to connect some other functionality (eg. visualizations such as a graph
 * or a heatmap) that can be applied to the filtered set of
...
public class StatSubmInfoFilterTable<S extends SubmissionInfo> {
	 ...
	 *            backing the table
	 * @param extraColumns
	 *            {@link List} of {@link SubmInfoColumnGenerator}s for
	 *            generating extra columns to the table; can be null
	 */
	...
	/**
	 * This method can be used to allow user to select certain rows that will be
	 * used in some further calculations or visualizations. If user has not
	 * explicitly selected any rows, this method defaults to returning all rows
	...
	/**
	 * Generates a list of {@link StatisticsInfoColumn}s from all the
	 * {@link StatisticalSubmissionInfo}-objects adding a column generated by
	 * each {@link SubmInfoColumnGenerator} registered to this instance.
	 * 
	 * @return {@link List} of {@link StatisticsInfoColumn}s generated by
	 *         registered {@link SubmInfoColumnGenerator}s from unfiltered data
	 */
	...
	/**
	 * <p>
	 * This method does the same as {@link #exportUnfilteredTabularData()} with
	 * the exception that it adds {@link StatisticsInfoColumn}s only for certain
	 * columns.
	 * </p>
	 * <p>
	 * Only columns generated by {@link SubmInfoColumnGenerator} for which
	 * {@link SubmInfoColumnGenerator #getColumnHeader(Localizer)
	 * getColumnHeader()} is in list of accepted titles are added to the
	 * generated list.
	 * </p>
	 * <p>
	 * This method can be useful when certain exercise-type has some uses
	 * {@link StatSubmInfoFilterTable} to show a more customized view on
	 * submissions made to it and wants to also return some or all of these
	 * custom columns from its
	 * {@link SubmissionStatisticsGiver #getAsTabularData()}-method.
	 * </p>
	 * 
	 * @param acceptedTitles
	 *            list of names that identify columns to export
	 * @return list of {@link StatisticsInfoColumn}s generated from accepted
	 *         columns
	 */
	 ...
	 * {@link StatSubmInfoFilterTable} and correct values for that column from
	...
	public interface SubmInfoColumnGenerator<T, A extends SubmissionInfo> {
		...
		/**
		 * This method tells whether it makes sense (or whether it is even
		 * possible) to export data generated by this
		 * {@link SubmInfoColumnGenerator} outside of Vaadin and ViLLE. An
		 * example of when to return false, is the 'visualize'-column.
		 * 
		 ...
		 * @return value for the column for row corresponding to given
		 *         statSubmInfo
		 */
	...
	/**
	 * <p>
	 * Helper method for parsing a {@link List} of
	 * {@link SubmInfoColumnGenerator}s usable with
	 * {@link StatSubmInfoFilterTable} from a list of
	 * {@link StatisticsInfoColumn}s.
	 * </p>
	 * <p>
	 * This method is analogous to
	 * {@link StatSubmInfoFilterTable #exportByColGenTitle(List)} in that it
	 * converts columns between the format used in
	 * {@link SubmissionStatisticsGiver #getAsTabularData()} and
	 * {@link StatSubmInfoFilterTable}. This time the conversion goes the other
	 * way around, meaning that if you already have implemented generation for
	 * some custom {@link StatisticsInfoColumn}s you can get those columns
	 * displayed in a {@link StatSubmInfoFilterTable} by using this method.
	 * </p>
	 * <p>
	 * The returned {@link SubmInfoColumnGenerator}s do not really do any hard
	 * work, they just map the already generated data and info from
	 * {@link StatisticsInfoColumn} in such a way that the resulting
	 * {@link TabularDataSetColumnGen} can act like a
	 * {@link SubmInfoColumnGenerator}.
	 * </p>
	 * 
	 * @param allSubmInfos
	 *            list of all {@link StatisticalSubmissionInfo}s (in correct
	 *            order)
	 * @param extraInfoCols
	 *            list of {@link StatisticsInfoColumn}s for which to generate
	 *            wrappers
	 * @return a list of {@link SubmInfoColumnGenerator} wrapping the
	 *         {@link StatisticsInfoColumn}s
	 */
	...
	/**
	 * A class for wrapping a {@link StatisticsInfoColumn} so that it can be
	 * used like {@link SubmInfoColumnGenerator}
	 * 
	 * @author Riku Haavisto
	 * 
	 * @param <A>
	...
	public static class TabularDataSetColumnGen<A extends Object, S extends SubmissionInfo>
		...
		/**
		 * Construct a new {@link TabularDataSetColumnGen} wrapping a
		 * {@link StatisticsInfoColumn} to {@link SubmInfoColumnGenerator}.
		 * 
		 * @param toWrap
		 *            {@link StatisticsInfoColumn} to wrap
		 * @param submInfosToOrigIndex
		 *            mapping from {@link StatisticalSubmissionInfo} to index
		 *            the object in original data-set; used to select correct
		 *            value from {@link StatisticsInfoColumn}s value when
		 *            "generating" columns to new rows
		 */
	...
	/**
	 * <p>
	 * Implements {@link SubmInfoColumnGenerator} in such a way that it
	 * generates a column with a button for opening a popup with
	...
	public static class ShowSubmissionColGenerator<E extends ExerciseData, S extends SubmissionInfo>
		...
		/**
		 * Constructs a {@link ShowSubmissionColGenerator} that can be added to
		 * {@link StatSubmInfoFilterTable} to generate a column with a button
		 * for opening a popup with {@link SubmissionVisualizer} for visualizing
		 * the {@link SubmissionInfo} corresponding to given row.
		 * 
		 * @param typeDesc
